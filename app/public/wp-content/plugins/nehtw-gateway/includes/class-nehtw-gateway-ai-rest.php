...
class Nehtw_Gateway_AI_REST {
    ...
    public function create_job( WP_REST_Request $request ) {
        ...
        $job_id = $this->extract_job_id_from_response( $api_response );

        if ( empty( $api_response['success'] ) || '' === $job_id ) {
            return new WP_REST_Response(
                array(
                    'success' => false,
    ...
    public function do_action( WP_REST_Request $request ) {
        ...
        $new_job_id = $this->extract_job_id_from_response( $api_response );

        if ( empty( $api_response['success'] ) || '' === $new_job_id ) {
            return new WP_REST_Response(
                array(
                    'success' => false,
    ...
    /**
     * Extract job_id from various possible API response shapes.
     */
    protected function extract_job_id_from_response( $api_response ) {
        if ( empty( $api_response ) || ! is_array( $api_response ) ) {
            return '';
        }

        $job_id = $this->locate_job_id_field( $api_response );
        if ( $job_id ) {
            return sanitize_text_field( $job_id );
        }

        if ( isset( $api_response['data'] ) && is_array( $api_response['data'] ) ) {
            $job_id = $this->locate_job_id_field( $api_response['data'] );
            if ( $job_id ) {
                return sanitize_text_field( $job_id );
            }
        }
        ...
        if ( isset( $api_response['result'] ) && is_array( $api_response['result'] ) ) {
            $job_id = $this->locate_job_id_field( $api_response['result'] );
            if ( $job_id ) {
                return sanitize_text_field( $job_id );
            }
        }

        $url_fields = array( 'get_result_url', 'result_url', 'status_url' );
        foreach ( $url_fields as $field ) {
            if ( ! empty( $api_response[ $field ] ) ) {
                $job_id = $this->job_id_from_url( $api_response[ $field ] );
                if ( $job_id ) {
                    return $job_id;
                }
            }
        ...
        if ( isset( $api_response['data'] ) && is_array( $api_response['data'] ) ) {
            foreach ( $url_fields as $field ) {
                if ( ! empty( $api_response['data'][ $field ] ) ) {
                    $job_id = $this->job_id_from_url( $api_response['data'][ $field ] );
                    if ( $job_id ) {
                        return $job_id;
                    }
                }
    ...
    /**
     * Try to fetch job ID directly from known key variations.
     */
    protected function locate_job_id_field( $data ) {
        $candidates = array( 'job_id', 'jobId', 'jobID', 'job' );
    ...
    /**
     * Attempt to derive job ID from a result URL (last path segment).
     */
    protected function job_id_from_url( $url ) {
        ...
        $job_id = end( $segments );
        return $job_id ? sanitize_text_field( $job_id ) : '';
    }
